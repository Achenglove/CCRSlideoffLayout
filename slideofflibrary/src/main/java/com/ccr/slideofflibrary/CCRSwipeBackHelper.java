...
public class CCRSwipeBackHelper {
    private Activity mActivity;
    private Delegate mDelegate;
    private CCRSwipeBackLayout mSwipeBackLayout;

    public CCRSwipeBackHelper(Activity activity, Delegate delegate) {
        mActivity = activity;
        mDelegate = delegate;
    ...
    private void initSwipeBackFinish() {
        if (mDelegate.isSupportSwipeBack()) {
            mSwipeBackLayout = new CCRSwipeBackLayout(mActivity);
            mSwipeBackLayout.attachToActivity(mActivity);
            mSwipeBackLayout.setPanelSlideListener(new CCRSwipeBackLayout.PanelSlideListener() {
                @Override
                public void onPanelSlide(View panel, float slideOffset) {
                    // 开始滑动返回时关闭软键盘
                    if (slideOffset < 0.03) {
                        CCRKeyboardUtil.closeKeyboard(mActivity);
                    }
     ...
     * @return
     */
    public CCRSwipeBackHelper setSwipeBackEnable(boolean swipeBackEnable) {
     ...
     * @return
     */
    public CCRSwipeBackHelper setIsOnlyTrackingLeftEdge(boolean isOnlyTrackingLeftEdge) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CCRSwipeBackHelper setIsWeChatStyle(boolean isWeChatStyle) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CCRSwipeBackHelper setShadowResId(@DrawableRes int shadowResId) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CCRSwipeBackHelper setIsNeedShowShadow(boolean isNeedShowShadow) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CCRSwipeBackHelper setIsShadowAlphaGradient(boolean isShadowAlphaGradient) {
        if (mSwipeBackLayout != null) {
     ...
     * @param threshold
     */
    public CCRSwipeBackHelper setSwipeBackThreshold(@FloatRange(from = 0.0f, to = 1.0f) float threshold) {
        if (mSwipeBackLayout != null) {
